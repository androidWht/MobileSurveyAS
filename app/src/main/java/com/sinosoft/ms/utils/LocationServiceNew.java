...
/*public class LocationServiceNew extends Service implements  Runnable
{
		...
		public void onLocationChanged(AMapLocation location)
		{
			if (null != location)
			{
				...
				if (location.getErrorCode() == 0)
				{
					...
					// 定位完成的时间
					// /sb.append("定位时间: " + Utils.formatUTC(location.getTime(),
					// "yyyy-MM-dd HH:mm:ss") + "\n");
				}
				else
				{
	...
	private void resetOption()
	{
		...
		// 设置是否开启wifi扫描，如果设置为false时同时会停止主动刷新，停止以后完全依赖于系统刷新，定位位置可能存在误差
			...
			{
				// 设置网络请求超时时间
				locationOption.setHttpTimeOut(Long.valueOf(6000));
			}
			catch (Throwable e)
			{
				e.printStackTrace();
